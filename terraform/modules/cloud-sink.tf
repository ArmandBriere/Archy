resource "google_logging_project_sink" "cloud_function_sink" {
  ...
  filter = "resource.type=\"cloud_function\" AND resource.labels.project_id=\"${var.project_id}\" AND severity=(ERROR OR CRITICAL OR ALERT OR EMERGENCY) AND resource.labels.function_name=\"^dev_*\""
